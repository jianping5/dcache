...
func startCacheServer(addr string, addrs []string, d *dcache.Group) {
	peers := dcache.NewHTTPPool(addr)
	peers.Set(addrs...)
	d.RegisterPeers(peers)
...
func main() {
	...
	d := createGroup()
	if api {
		go startAPIServer(apiAddr, d)
	}
	startCacheServer(addrMap[port], []string(addrs), d)
}